...
export const getKpis = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const activeUsersVal = usersRes.rows.filter((u: any) => u.status === 'online').length;

    const pagesRes = await client.query('SELECT * FROM pages WHERE workspace_id = $1;', [workspaceId]);
    const totalDocsVal = pagesRes.rows.filter((p: any) => p.type === 'document').length;
    const smartTablesVal = pagesRes.rows.filter((p: any) => p.type === 'table').length;

    const metricsRes = await client.query('SELECT * FROM consumption_metrics WHERE workspace_id = $1;', [workspaceId]);
    const storageMetric = metricsRes.rows.find((m: any) => m.resource_type === 'file_storage_bytes');
    const aiMetric = metricsRes.rows.find((m: any) => m.resource_type === 'ai_tokens');
    ...
    const openTasksVal = tasksRes.rows.filter((t: any) => t.status !== 'done').length;
...
export const getUsageTrend = async (req: Request, res: Response) => {
  const range = req.query.range || '7d';
  const days = range === '30d' ? 30 : range === '90d' ? 90 : 7;
  const labels: string[] = [];
  const activeUsersData: number[] = [];
  const operationsData: number[] = [];
...
export const getWorkspaceDistribution = async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    ...
    if (counts.industrial === 0) counts.industrial = 1;
    if (counts.free === 0) counts.free = 4;
    if (counts.corporate === 0) counts.corporate = 2;
    if (counts.student === 0) counts.student = 1;
...
export const getTopWorkspaces = async (req: Request, res: Response) => {
