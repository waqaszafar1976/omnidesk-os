...
const authorizeDeveloper = async (req, res, next) => {
...
router.get('/developer/pages', authorizeDeveloper, async (req, res) => {
    ...
    `);
        ...
        await (0, billing_controller_1.logAuditEvent)(client, userId, 'API_GET_PAGES', 'developer: pages list retrieved', { source: 'api_token' });
...
router.post('/developer/pages', authorizeDeveloper, async (req, res) => {
    const userId = req.headers.user_id;
    ...
    `, [workspaceId]);
        // Audit Log API access
        await (0, billing_controller_1.logAuditEvent)(client, userId, 'API_CREATE_PAGE', `developer: created page "${page.title}"`, { source: 'api_token' });
        await client.query('COMMIT');
